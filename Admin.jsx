...
export default function Admin({ logout }) {
  ...
  useEffect(() => {
    ...
    // الإضافة هنا: الأدمن بيسمع أول ما اللعبة تبدأ فعلياً
  socket.on("gameStarted", (images) => {
    setStartedImages(images); // لما دي تتحدث، الأدمن هينتقل لصفحة البازل
  });
    ...
    socket.off("gameStarted"); // تنظيف المستمع عند الخروج
  };
}, []);
...
const r = await axios.post("https://asset-manager--bdallahashrf110.replit.app/upload", fd, {
  ...
  if (startedImages) {
  return <Puzzle images={startedImages} playerName="Admin" />;
}
